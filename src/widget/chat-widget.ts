...
import {  IntentRequest,LOCATION_REQUEST, SELECTION_REQUEST  } from '../models/IntentRequest';
...
export class ChatWidgetElement extends HTMLElement {
  ...
  protected chatState: boolean = true;
  ...
  connectedCallback() {
    ...
    const phonenumber = this.getAttribute("phonenumber") || "";
    ...
    // Listen to list item selected
    //@ts-ignore
    window.addEventListener("listItemSelected", (event: CustomEvent) => {
      // retrieve custom data
      const message = event.detail as SELECTION_REQUEST;
      ...
      // set the chat message to the intent request and validate
      this.intentRequest.query = message;
      this.intentRequest.requestType = "SELECTION";
